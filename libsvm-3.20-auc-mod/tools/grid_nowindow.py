...
def redraw(db,best_param,gnuplot,options,tofile=False):
	...
	if tofile:
		...
		gnuplot.write(b"set term wxt noraise\n")

	else:
		gnuplot.write( b"set term x11\n")
		gnuplot.write(b"set term x11 noraise\n")
...
def find_parameters(dataset_pathname, options=''):
	...
	if options.gnuplot_pathname:
		...
		gnuplot = subprocess.Popen(options.gnuplot_pathname,stdin = PIPE,stdout=PIPE,stderr=PIPE, startupinfo=startupinfo).stdin
	else:
		gnuplot = None
